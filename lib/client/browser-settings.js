...
function init (client, serverSettings, $) {
  ...
  function loadForm () {
    ...
    var tzSelect = $('#timezone-browser');
    // Populate the IANA zone list once (the static 'auto' option is already present).
    if (tzSelect.length && tzSelect.children('option').length <= 1) {
      _.each(client.timezones || [], function eachZone (zone) {
        tzSelect.append('<option value="' + zone + '">' + zone + '</option>');
      });
    }
    tzSelect.val(settings.displayTimezone || 'auto');
  ...
  function wireForm () {
    ...
    $('#save').click(function(event) {
      ...
      storeInBrowser({
        ...
        , displayTimezone: $('#timezone-browser').val()
        , scaleY: $('#scaleY').val()
