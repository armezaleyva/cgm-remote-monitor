...
function init (client, d3, $) {
  ...
  var tickFormat = function(date) {
    // Boundary classification stays on the real (device-local) date; only the
    // rendered label is shifted into the selected display timezone. For the
    // whole-hour-offset zones this covers (e.g. US zones), tick placement and
    // labels stay aligned to the hour.
    var labelDate = client.tzAdjustedDate ? client.tzAdjustedDate(date) : date;
