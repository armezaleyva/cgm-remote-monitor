...
var client = {};
client.timezones = timezones; // exposed early: the settings form wires up (loadAndWireForm) before the later client.* assignments below
...
client.load = function load (serverSettings, callback) {
  ...
  client.tzAdjustedDate = tzAdjustedDate;
  ...
  // Shift a date so that formatting it with the browser's local timezone
  // yields the wall-clock time of the user-selected display timezone.
  // For 'auto' (or unset) it is a no-op, preserving the viewer's device time.
  // Display-only: never feed the result back into data or calculations.
  function tzAdjustedDate (time) {
    var tz = client.settings.displayTimezone;
    if (!tz || tz === 'auto') { return time; }
    var date = (time instanceof Date) ? time : new Date(time);
    var targetOffset = moment(date).tz(tz).utcOffset(); // minutes east of UTC in the target zone
    var localOffset = -date.getTimezoneOffset();        // minutes east of UTC in the browser
    return new Date(date.getTime() + (targetOffset - localOffset) * 60000);
  }

  function formatTime (time, compact) {
    var timeFormat = getTimeFormat(false, compact);
    time = d3.timeFormat(timeFormat)(tzAdjustedDate(time));
